...
namespace Natty.Utility.ToolBox
{
    ...
    public sealed class Character
    {
        ...
        /// Longs to string
        /// </summary>
        /// <param name="aryLong"></param>
        /// <param name="SeparateChar"></param>
        /// <returns></returns>
        public static string ArrayToString(long[] aryLong, string SeparateChar)
        {
            StringBuilder str = new StringBuilder();

            foreach (long s in aryLong)
            {
                if (!(SeparateChar + str.ToString()).Contains(SeparateChar + s + SeparateChar))
                {
                    str.Append(s);
                    str.Append(SeparateChar);
                }
        ...
        #region 单条Json数据转数组
        public static List<string> JsonItemList(string strjson)
        {
            ...
            try
            {
                ...
                {
                    string str = strjson.Substring(strjson.IndexOf("{") + 1, strjson.IndexOf("}") - strjson.IndexOf("{") - 1);
                    int start = 0;
                    int at = 0;
                    int end = str.Length;
                    int count = 0;
                    ...
                    while ((start <= end) && (at > -1))
                    {
                        ...
                        if (at == -1)
                        {
                            one = str.Substring(start, end - start);
                        }
                        else
                        {
                            if (tag)
                            {
                                one = str.Substring(start, at - start);
                            }
                            else
                            {
                                one = one + str.Substring(start, at - start);
                                tag = true;
                            }
                        }
                        if (one.Split('\"').Length % 2 == 1)
                        {
                            if (one.IndexOf(":") > 0)
                            {
                                //string left = one.Substring(0, one.IndexOf(":"));
                                //string right = one.Substring(one.IndexOf(":") + 1, one.Length - one.IndexOf(":") - 1);
                                //restr = restr + "at:" + at + "length:" + one.Split('\"').Length + "left:" + left + "right:" + right + "<br />";
            ...
            {
                strlist.Clear();
                strlist.Add(strjson.Replace("\"",""));
                strlist.Add("转化失败,原值去引号输出");
            }
